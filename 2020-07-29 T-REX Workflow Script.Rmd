...
```{r KNN,out.height=10,out.height=10}
# KNN search per cell 
...
all.data = cbind(my.sampled.data, umap.data)
...
trex.plot$cuts = cut(trex.plot$col, c(0, 5, 15, 85, 95, 100), include.lowest = TRUE)
...
final_trex_plot <-
  ...
  labs(caption = "Data from Brodin et al., medRxiv 2020")
...
build.heatmaps(MEM.values.KNN, cluster.MEM = "none", cluster.medians = "none",
               display.thresh = 1, newWindow.heatmaps=TRUE, output.files = TRUE, labels = FALSE)
