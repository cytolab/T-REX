...
output_location = "data_files/brodin covid/"
if (!file.exists(paste0(output_location, output_dir))) {
  dir.create(paste0(output_location, output_dir))
}
```
...
# uncomment the line below to save UMAP 
# save_uwot(myumap, paste0(getwd(), "/", output_location, output_dir, strftime(Sys.time(),"%Y-%m-%d_%H_%M"),"_umap_results"))
...
```{r}
# generate bin sequences (remove later)

fifty_bins = vector()
for (i in seq(from = 0, to = 99, by = 2)) {
  this_str = paste0("[",i,",",i + 2,")")
  fifty_bins <- append(fifty_bins, this_str)
}
fifty_bins[length(fifty_bins)] <- "[98,100]"
```


```{r run TREX}
source("R/TREX.R")
setwd(paste0(getwd(),"/data_files/brodin covid/output files"))

my.TREX <- TREX(
  embedding.data = cbind(umap.data, file_ID = sampled.data$file_ID),
  
  # Hannah's extended bins 
  # bins = c('[0,5)','[5,15)','[15,25)','[25,37.5)','[37.5,42.5)','[42.5,47.5)','[47.5,52.5)',
  #          '[52.5,57.5)','[57.5,62.5)','[62.5,75)','[75,85)','[85,95)','[95,100]')
  
  # 19 bins to test extended colors 
  # bins = c('[0,5)','[5,10)','[10,15)','[15,20)','[20,25)','[25,30)','[30,35)',
  #          '[35,40)','[40,45)','[45,55)','[55,60)','[60,65)','[65,70)',
  ...
  # even # of bins 
  # bins = c('[0,5]','(5,15]','(15,35]','(35,85)','[85,95)','[95,100]')
)
...
#   embed.type = "UMAP",
#   percent.labels = TRUE,
#   # title = "Timepoint 1 vs. Timepoint 2",
...
my.clusters <- TREX_cluster(my.TREX, transformed.data)

# marker.data is an optional argument for keeping track of the measured marker data 
# for use downstream (such as in MEM) when TREX selects populations of interest 
# it is not used in the calculation of TREX 
```
...
png(
  paste0(output_location, output_dir, strftime(Sys.time(),"%Y-%m-%d_%H_%M"), "_marker_heat.png"),
  height = 2000,
  width = 4000
)
