...
my.chosen.sampled.data = my.sampled.data[,c(5:31)]
colnames(my.chosen.sampled.data) 
transformed.data = as.data.frame(t(apply(my.chosen.sampled.data, 1, function(x) asinh(x/15))))
...
trex.plot <- data.frame(x = umap.data[, 1], y = umap.data[, 2], col = test.round)
...
colnames(all.data)[1:2]<-orig.names[1:2]
colnames(all.data)[4]<-"Barium2"
colnames(all.data)[35]<-"Lead2"
colnames(all.data)[36]<-"Lead3"
...
dbscan_plot <- 
  ...
  labs(x = "UMAP 1", y = "UMAP 2", title = "DBSCAN Clusters (5th & 95th percentiles)", color = "DBSCAN Cluster") +
  theme_bw() + 
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        legend.title = element_blank())
...
MEM.data = track.data[,c(5:31,ncol(track.data))]
MEM.data = MEM.data[order(MEM.data$cluster),]
MEM.values.KNN = MEM(MEM.data, transform = TRUE, cofactor = 15,
                     choose.markers = FALSE, markers = "all", choose.ref = FALSE, zero.ref = TRUE, rename.markers = FALSE,
                     new.marker.names = "CD235a,CD19,CD117,CD11b,CD4,CD64,CD7,CD34,CD61,CD123,CD13,CD62L,CD45,CD183,CD33,CD11c,CD14,CD15,CD16,CD24,CD38,CD25,CD3,CD185,HLA-DR,CD184,CD56", file.is.clust = FALSE, add.fileID = FALSE, IQR.thresh = NULL,scale.matrix = "arcsinh",scale.factor = 1)
